...
```{r}
DT::datatable(head(select(dt, title, author, date, publisher, `container-title`, doi), n = 50), rownames = F)
```
...
```{r}
DT::datatable(head(select(cleaned_dt, title, author, year, publisher, `container`, doi), n = 50), rownames = F)
```

Let's save this file for use in our next step in the workflow, the [citationClassify](https://github.com/govscienceuseR/citationClassify) package. 
```{r, eval = F}
