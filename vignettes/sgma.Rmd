...
packs = c('data.table','pdftools','pbapply','stringr', 'jsonlite', 'purrr', 'magrittr')
...
```{r}
citation_extract(doc_dir = 'documents_gsp/', 
                 ref_dir = 'reference_extracts_gsp', 
                 layout = "none")
```

Now we have the citation JSONS.
```{r}
list.files('reference_extracts_gsp')
```
...
```{r, eval = F}
dt <- citation_compile('reference_extracts_trial1_nolayout')
```
